...
object LineOfSight {
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    def dsHelper(idx: Int, until: Int, maxim: Float): Unit = {
      if(idx < until) {
        output(idx) = List(input(idx) / idx, maxim).max
        dsHelper(idx + 1, until, output(idx))
      }
    }
    dsHelper(from, until, startingAngle)
  }
