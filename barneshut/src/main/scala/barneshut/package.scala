...
package object barneshut {
  ...
  case class Fork(nw: Quad, ne: Quad, sw: Quad, se: Quad) extends Quad {
    ...
    val mass: Float = nw.mass + sw.mass + se.mass + ne.mass
    val massX: Float = if (mass != 0)
      (nw.massX * nw.mass + ne.massX * ne.mass + sw.massX * sw.mass + se.massX * se.mass) / mass else centerX
    val massY: Float = if (mass != 0)
      (nw.massY * nw.mass + ne.massY * ne.mass + sw.massY * sw.mass + se.massY * se.mass) / mass else centerY
    val total: Int = nw.total + sw.total + se.total + ne.total
  ...
  extends Quad {
    ...
    val massX: Float = bodies.map(b => b.mass * b.x).sum / bodies.map(b => b.mass).sum
    val massY: Float = bodies.map(b => b.mass * b.y).sum / bodies.map(b => b.mass).sum
    ...
    def updated(quad: Quad): Body = {
      ...
      def traverse(quad: Quad): Unit = (quad: Quad) match {
        ...
        case Leaf(_, _, _, bodies) =>
          for(body <- bodies) addForce(body.mass, body.x, body.y)
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val xSector = Math.min(((b.x - boundaries.minX) / (boundaries.width / sectorPrecision)).toInt,
                              (boundaries.width / (boundaries.width / sectorPrecision)).toInt - 1)
      val ySector = Math.min(((b.y - boundaries.minY) / (boundaries.height / sectorPrecision)).toInt,
                              (boundaries.height / (boundaries.height / sectorPrecision)).toInt - 1)
      this.apply(xSector, ySector) += b
      this
    }
